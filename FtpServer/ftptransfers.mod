...
TYPE
    ...
    (*                   permissions.                                           *)
    (*   host            Hostname as supplied in HOST command.                  *)
...
PROCEDURE SetHost (SS: ClientFileInfo;  hostname: ARRAY OF CHAR): BOOLEAN;
    ...
    BEGIN
        IF SS = NIL THEN
            RETURN FALSE;
        ELSE
            Strings.Assign (hostname, SS^.host);
            RETURN TRUE;
        END (*IF*);
    END SetHost;
...
PROCEDURE FindUser (Name: ARRAY OF CHAR;
    ...
    BEGIN
        WITH SessionInfo^ DO
            DiscardFName (iname);
            user := ReadUserData (Name, host, SpeedLimit, UpSpeedLimit,
            ...
            IF category <> NoSuchUser THEN
                ...
                IF host[0] <> Nul THEN
                    Strings.Append ("@", Log.UserName);
                    Strings.Append (host, Log.UserName);
                END (*IF*);
                LogIt := LogSession;
            END (*IF*);
        END (*WITH*);
